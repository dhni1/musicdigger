# CHANGELOG

## v0.1.0
- 파일 생성

## v0.1.1
- ui 생성

## v0.1.2
- v0.1.1 갈아엎고 ui 및 기능 제작

## v0.1.3
- v0.1.2에서 ui 디자인 수정

## v0.1.4
- v0.1.3에서 ui 디자인 수정

## v0.1.5
- v0.1.4에서 ui 디자인 수정

## v0.2.0
- spotify 연결 시도

## v0.2.1
- spotify 연결 실패 후 장르에 붐뱁 추가

## v0.3.0
- Spotify 장르/트랙 카탈로그 조회용 Python 서버 추가
- 프론트에서 `/api/genres`, `/api/genre-details` 우선 사용하도록 변경
- 서버 미연결 시 `genres.json`으로 자동 폴백되도록 수정
- Spotify 플레이리스트/좋아요/프로필 UI 유지하면서 백엔드 기반 장르 탐색 구조 추가

## v0.3.1
- Spotify 연결 안되는 문제 수정

## v0.3.2
- Spotify 연결 안되는 문제 수정

## v0.4.0
- backend

## v0.4.1
- frontend

## v0.4.2
- error

## v0.4.3
- error

## v0.4.4
- 프론트 스크립트 중복 로딩 문제 수정
- DOM 로딩 전 이벤트 바인딩으로 발생하던 콘솔 에러 수정
- 백엔드 `/api/genres`, `/api/genre-details` 라우팅 추가
- Render 배포 환경에서 API가 404를 반환하던 문제 수정

## v0.4.5
- Spotify 장르 시드 API 의존성을 제거하고 `data/genres.json` 기반으로 장르 목록 응답하도록 수정
- `/api/genres` 호출 시 502가 발생하던 문제 수정

## v0.4.6
- 검색창이 트랙명이나 아티스트명이 아니라 장르 이름과 스타일만 검색하도록 수정
- Spotify `Get Available Genre Seeds` 와 `Get Recommendations` 흐름을 기준으로 장르별 추천 곡을 우선 불러오도록 수정
- 추천 API 실패 시 로컬 장르 데이터와 장르 기반 검색 결과로 자연스럽게 폴백되도록 보강

## v0.4.7
- `Boom Bap`, `R&B`, `Lo-Fi`, `K-Pop` 같은 장르 별칭을 Spotify seed와 매핑하도록 보강
- 장르 검색 시 별칭, seed 이름, 검색어까지 함께 인식하도록 수정
- `data/genres.json`에 Spotify 기반 탐색용 장르를 대폭 추가하고 연결 장르(`dance`, `grime`, `rap-rock`, `drill`)도 함께 보강

## v0.5.0
- 홈 화면 장르 라이브러리를 기본 4개만 노출하고 `All Genres` 버튼으로 전체 확장되도록 수정
- `Library`와 `Profile`을 스크롤 섹션이 아닌 별도 화면 전환 방식으로 개편
- 프로필 버튼 클릭 시 Spotify 계정 상태와 라이브러리 요약을 보는 프로필 화면으로 이동하도록 수정
- 장르 카드의 `recommendation picks` 문구와 waveform 패널 제거
- `Jazz Hip-Hop` 설명이 Spotify 아티스트 문구로 덮어써지지 않도록 수정하고 기본 대표 곡 수 보강
- `K-Pop` seed/search 기준과 대표 곡을 한국 아이돌 음악 맥락에 맞게 조정

## v0.5.1
- 장르 상태 표시에서 `4 / 23 장르` 같은 축약 카운트 문구 제거
- 사이드바에서 `Liked Tracks`, `Search` 메뉴를 제거하고 `Playlists`를 생성 진입점으로 유지
- 라이브러리 화면의 `Create Playlist` 버튼 제거
- 프로필 화면의 액션 버튼을 제거하고 `Go To` 영역을 `Settings` 영역으로 변경
- 상단 메뉴 패널은 `Settings` 한 개만 노출되도록 정리

## v0.5.2
- 장르 상세 트랙 로딩 기준을 Spotify 추천 위주에서 인기순 검색 기준으로 조정
- Spotify 검색 결과와 추천 결과를 합쳐 중복 제거 후 장르별 대표곡 8개를 우선 노출하도록 수정
- 트랙 popularity 값을 함께 보존해 더 유명한 곡이 앞에 오도록 정렬
- 홈 히어로 영역의 라이트/다크 모드 버튼 제거
- `Settings`를 프로필과 분리된 별도 화면으로 추가
- 프로필 화면 `Settings` 칸에 `More Settings` 버튼을 추가해 설정 화면으로 이동하도록 수정
- 설정 화면에서 라이트/다크 모드 전환을 별도로 제공하고 프로필 내 간단 전환 버튼은 유지

## v0.5.3
- 장르 라이브러리의 기본 `전체 장르` 상태 텍스트를 숨기도록 조정
- 확장 버튼 라벨을 `All Genres`에서 `Show More`로 변경
- 장르별 대표 곡 선정 시 Spotify popularity 기준으로 상위 8곡이 더 정확히 앞에 오도록 정렬 로직 보강
- 검색 결과와 추천 결과를 합친 뒤 다시 popularity 기준으로 재정렬하도록 수정
- `Representative Tracks` 섹션명을 `Representative Major Tracks`로 변경
- `Subgenres`, `Similar`, `Fusion` 연결 카드가 내용과 상관없이 비슷한 크기로 유지되도록 높이 고정 스타일 추가
- 빈 상태 박스와 장르 옵션 영역도 카드 내부에서 일정한 높이를 유지하도록 정리

## v0.5.4
- `data/genres.json`의 모든 장르 폴백 데이터에 대표곡 8개씩 채워 넣도록 보강
- JSON 로드 실패 시를 대비한 내장 기본 장르 데이터도 대표곡 8개 기준으로 확장
- Spotify 백엔드 연결 상태에서도 곡 수가 부족한 장르는 로컬 대표곡과 병합해 8곡을 채우도록 보강
- Spotify 결과가 2곡만 내려오는 경우에도 장르별 대표곡 리스트가 8개를 유지하도록 수정
- Spotify 연결 시 대표곡을 더 적극적으로 확보하도록 `artist top tracks` 수집 단계 추가
- 장르별 대표곡 선정 우선순위를 Spotify 검색, 추천, 아티스트 top tracks 중심으로 조정
- 로컬 대표곡은 Spotify 결과를 전혀 확보하지 못했을 때만 마지막 폴백으로 사용하도록 변경

## v0.5.5
- 앱 시작 시 `data/genres.json` 장르를 먼저 즉시 렌더하고 백엔드 장르 카탈로그는 뒤에서 갱신하도록 변경
- Render 백엔드 응답이 느릴 때도 첫 장르 카드와 대표곡이 바로 보이도록 프론트 로딩 흐름 개선
- `/api/genres`가 Spotify seed 조회를 기다리지 않고 로컬 장르 카탈로그를 즉시 반환하도록 정리
- 장르 상세 조회 시 Spotify 검색/아티스트 조회를 필요한 수만큼만 수행하도록 서버 호출 수를 줄여 체감 로딩 속도 개선

## v0.5.6
- `Show More` 버튼 옆에 남아 있던 빈 상태 배지 흔적 제거

## v0.5.7
- 맵 배치 로직을 힘 기반 이동 방식 대신 정적 좌표 방식으로 정리
- `subgenre`, `fusion`에 속하지 않는 장르를 `코어 장르`로 간주해 더 중심 영역에 배치
- 코어 장르는 중심에 두되 한곳에 몰리지 않도록 분산시키고 파생 장르는 바깥쪽으로 더 넓게 배치
- 히어로 영역의 `Genre Focus` 옆 보조 배지 제거로 상단 UI 정리

## v0.6.0
- 왼쪽 사이드바에 `Map` 메뉴 추가
- `Every Noise at Once` 스타일을 참고한 장르 맵 뷰 추가
- 장르 이름이 군집별로 퍼져 보이는 타이포 중심 탐색 화면 구현
- 현재 선택한 장르와 연결된 `Subgenres`, `Similar`, `Fusion` 흐름을 맵에서 강조
- 맵 우측 패널에 선택 장르 설명, 연결 장르, 대표곡 미리보기 추가

## v0.6.1
- 선택 상태 배지 문구를 `Music Map`으로 정리
- 맵 내부 캔버스를 더 크게 확장하고 화면에는 고정된 뷰포트만 보이도록 구성
- 드래그 및 스크롤로 큰 장르 지도를 이동하며 탐색할 수 있도록 수정
- 장르 글씨가 서로 겹치지 않도록 배치 로직 보강
- `Open Full Map` 버튼과 전체 화면에 가까운 맵 모달 추가

## v0.6.2
- full map 크기 조정
- `UI_TERMS.md` 추가

## v0.6.3
- full map 크기 조정
- genre atlas 창 제거

## v0.6.4
- full map 크기 조정

## v0.6.5
- 버그 수정

## v0.6.6
- 버그 수정

## v0.7.0
- css, javascript 페이지별로 분리
- css, javascript 페이지별로 분리

## v0.7.1
- `script.js`, `design.css`는 엔트리만 남기고 `src/`, `styles/` 기준으로 구조를 정리
- `home`, `map`, `library`, `profile` 화면 로직을 페이지별 모듈로 분리
- 동적 렌더링에서 `innerHTML` 사용을 제거하고 DOM API 기반으로 보안 보강
- Spotify 클라이언트 토큰 저장소를 `localStorage`에서 `sessionStorage`로 변경
- 백엔드에서 저장소 루트 전체가 노출되지 않도록 정적 파일 서빙 범위를 제한
- `/api/genre-details` 입력값 검증, 캐시 상한, rate limiting, CORS 제한 추가
- CSP, `X-Frame-Options`, `X-Content-Type-Options` 등 기본 보안 헤더 추가
- `README.md`에 현재 폴더 구조와 보안 관련 변경사항 반영

## v0.7.2
- route fallback 추가

## v0.7.3
- 홈 화면 히어로 상단 박스를 제거하고 배경 위에 텍스트가 바로 놓이는 형태로 정리
- 히어로 통계(`Genres`, `Tracks`, `Connections`) 영역을 더 가벼운 스타일로 조정
- 사이드바 `Playlists`를 URL 이동이 아닌 플레이리스트 생성 모달 진입 방식으로 복원
- `/feed/playlists` 전용 라우팅과 서버 fallback 제거

## v0.7.4
- 프로필 화면에 `Disconnect Spotify` 버튼을 추가하고 Spotify 세션/라이브러리 상태를 바로 초기화할 수 있도록 수정
- 햄버거 버튼의 `Settings` 드롭다운을 제거하고 왼쪽 오프캔버스 사이드바를 여닫는 방식으로 변경
- 사이드바가 기본적으로 숨겨져 있다가 햄버거 버튼 클릭 시 왼쪽에서 슬라이드되어 나타나도록 조정
- 사이드바 외부 클릭과 `Esc` 키로 사이드바를 닫을 수 있도록 보강
- `Random Dig` 버튼을 사이드바에서 상단 검색 영역 오른쪽으로 이동
- 검색창이 상단바 중앙에 더 안정적으로 배치되도록 레이아웃 조정

## v0.7.5
- `Open Full Map` 모달이 사이드바 개편 이후에도 상단바 아래 전체 폭을 다시 꽉 채우도록 레이아웃 수정

## v0.8.0
- `data/genres.json` 장르 카탈로그를 `23개 -> 47개`로 대폭 확장
- `Neo Soul`, `City Pop`, `Disco`, `Funk`, `Shoegaze`, `Emo`, `Synthwave`, `Afrobeats`, `Amapiano`, `Reggaeton`, `Soundtrack` 등 장르 축 추가
- 기존 장르의 `Subgenres`, `Similar`, `Fusion` 연결을 새 카탈로그 기준으로 다시 정리해 탐색 흐름 보강
- 장르 검색 시 더 다양한 스타일 키워드와 연결 장르가 함께 잡히도록 메타데이터 확장
- 장르 맵 군집 키워드를 넓혀 신규 장르가 특정 구역에 몰리지 않고 더 자연스럽게 분산되도록 조정

## v0.8.1-0.8.2
- ui 수정

## v0.8.3~5
- MAP UI 수정

## v0.8.6
- 맵 화면을 카드형 패널보다 큰 장르 필드 중심 레이아웃으로 재구성
- 맵 우측 정보 패널을 장르 클릭 시 열리는 드래그 가능한 팝업 형태로 변경
- 팝업에 `Close` 버튼과 `Esc` 닫기 동작 추가
- 맵 하단 재생바 제거
- 맵 상단 안내 문구 제거 및 컨트롤을 맵 위 오버레이 형태로 정리
- `Open Full Map` 버튼 제거

## v0.8.7
- 맵 화면 상단 여백을 줄이고 지도 자체가 더 위에 붙어 보이도록 레이아웃 조정
- `Visible`, `Links`, 확대/축소, `Reset` 컨트롤을 맵 위에 떠 있는 오버레이 형태로 변경
- 맵 첫 진입 시 전체 장르 필드가 한눈에 들어오도록 자동 맞춤 줌과 중앙 정렬 추가
- 화면에 딱 맞는 기본 지도 상태를 `100%` 기준으로 재정의하고 그보다 더 축소되지 않도록 수정
- 축소된 기본 화면에서도 장르명과 클러스터 라벨 크기가 과하게 커지지 않도록 타이포 스케일 보정

## v0.8.8
- 맵 메인 화면이 바깥 페이지 스크롤 없이 화면 안에서 꽉 차도록 높이 레이아웃을 flex 기반으로 재조정
- 맵 상단 오버레이 컨트롤을 오른쪽 상단 정렬로 옮기고 검은 배경/테두리 느낌을 줄여 더 가볍게 보이도록 수정
- 맵 첫 진입 시 실제 화면 크기가 잡힌 뒤 자동 맞춤 줌이 적용되도록 초기 계산 타이밍 보정
- 맵 `Reset`이 초기 맞춤 배율 기준 `100%`로 돌아가도록 수정하고 컨트롤 위치를 화면 오른쪽 상단 고정 기준으로 조정

## v0.8.9
- 오류 수정

## v0.8.10
- UI 수정

## v0.8.11
- UI 수정

## v0.9.0
- 맵 화면을 Everynoise 스타일에 가까운 유사도 기반 장르 지도 구조로 개편
- 장르 배치를 고정 군집명이 아닌 텍스트/연결관계 기반 2D 투영 로직으로 재설계
- 맵 노드 선택 팝업의 대표곡 수를 3개로 줄이고 연결선 표시를 제거해 화면을 더 단순하게 정리
- 맵 오버레이 컨트롤을 오른쪽 상단 고정 형태로 다듬고 기본 줌 상태가 항상 `100%`로 보이도록 보정
- 맵 노드 스타일을 기본 흰색, 선택 시 노란색 강조 방식으로 통일

## v0.9.1
- 홈 `Genre Library`를 `Show More` 확장 방식 대신 페이지네이션 방식으로 변경
- 장르 카드를 한 번에 8개씩 표시하고 `Prev` / `Next`로 넘길 수 있도록 조정
- 현재 선택한 장르가 다른 페이지에 있으면 해당 페이지로 자동 이동하도록 보완

## v0.9.2
- 8개를 10개로 변경

## v0.9.3~7
- map 변경
- 맵 배치를 코어 장르 중심의 정적 마인드맵 스타일로 재구성
- 파생 장르와 융합 장르가 부모 장르에서 바깥쪽 가지처럼 뻗도록 정리
- 장르를 연속해서 눌러도 팝업이 마지막으로 클릭한 노드 위치에서 다시 열리도록 수정
- 장르 사이 관계 강도에 따라 정적으로 더 가깝거나 멀어지도록 배치 보정 추가
- 노드 간 기본 간격을 넓혀 한쪽에 뭉치지 않도록 맵 분포 확장
- 코어 장르는 더 넓게 벌리고 관계가 약한 장르는 더 멀어지도록 전체 맵 스프레드와 거리 기준 추가 조정
