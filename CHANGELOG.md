...
## v0.7.4
- 프로필 화면에 `Disconnect Spotify` 버튼을 추가하고 Spotify 세션/라이브러리 상태를 바로 초기화할 수 있도록 수정
- 햄버거 버튼의 `Settings` 드롭다운을 제거하고 왼쪽 오프캔버스 사이드바를 여닫는 방식으로 변경
- 사이드바가 기본적으로 숨겨져 있다가 햄버거 버튼 클릭 시 왼쪽에서 슬라이드되어 나타나도록 조정
- 사이드바 외부 클릭과 `Esc` 키로 사이드바를 닫을 수 있도록 보강
- `Random Dig` 버튼을 사이드바에서 상단 검색 영역 오른쪽으로 이동
- 검색창이 상단바 중앙에 더 안정적으로 배치되도록 레이아웃 조정
...
## v0.8.6
- 맵 화면을 카드형 패널보다 큰 장르 필드 중심 레이아웃으로 재구성
- 맵 우측 정보 패널을 장르 클릭 시 열리는 드래그 가능한 팝업 형태로 변경
- 팝업에 `Close` 버튼과 `Esc` 닫기 동작 추가
- 맵 하단 재생바 제거
- 맵 상단 안내 문구 제거 및 컨트롤을 맵 위 오버레이 형태로 정리
- `Open Full Map` 버튼 제거

## v0.8.7
- 맵 화면 상단 여백을 줄이고 지도 자체가 더 위에 붙어 보이도록 레이아웃 조정
- `Visible`, `Links`, 확대/축소, `Reset` 컨트롤을 맵 위에 떠 있는 오버레이 형태로 변경
- 맵 첫 진입 시 전체 장르 필드가 한눈에 들어오도록 자동 맞춤 줌과 중앙 정렬 추가
- 화면에 딱 맞는 기본 지도 상태를 `100%` 기준으로 재정의하고 그보다 더 축소되지 않도록 수정
- 축소된 기본 화면에서도 장르명과 클러스터 라벨 크기가 과하게 커지지 않도록 타이포 스케일 보정
